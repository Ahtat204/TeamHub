...
namespace TeamcollborationHub.server.Services.RateLimiting
{
    public class IpBasedRateLimiter : IRateLimiter
    {
        ...
        private readonly LuaScript _luaScript;// this Lua script will be used to atomically check and update the request count for each IP address in Redis. It will return 1 if the request is allowed, and 0 if the rate limit has been exceeded.
        ...
        private readonly int _windowSizeInSeconds;// this is the time window for which the rate limit is applied. For example, if this is set to 60, then the rate limit will be applied for each 60 seconds window.
        ...
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
           ...
           var allowed=(int) await _redisDatabase.ScriptEvaluateAsync(_luaScript.ToString(), new RedisKey[] { ip }, new RedisValue[] { _maxRequests, _windowSizeInSeconds });
            if (allowed == 0) 
            {
